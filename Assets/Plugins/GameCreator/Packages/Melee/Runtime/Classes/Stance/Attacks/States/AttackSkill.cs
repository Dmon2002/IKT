...
namespace GameCreator.Runtime.Melee
{
    public class AttackSkill : TAttackState
    {
        ...
        private void UpdateMotionWarp(Character self, float elapsedRatio)
        {
            ...
            {
                this.m_StartLocationSelf = new Location(self.Feet, self.transform.rotation);
                this.m_StartLocationTarget = new Location(
                    target != null ? target.Feet : this.m_Args.Target != null
        ...
        private void UpdateWarpLocation(Character character, Location start, Location warp, float ratio)
        {
            if (warp.HasPosition)
            {
                ...
                character.Driver.SetPosition(Vector3.Lerp(
                    start.GetPosition(character.gameObject),
                    warp.GetPosition(character.gameObject), 
                    ratio
                ));
            }
            ...
            if (warp.HasRotation)
            {
                ...
                character.Driver.SetRotation(Quaternion.Lerp(
